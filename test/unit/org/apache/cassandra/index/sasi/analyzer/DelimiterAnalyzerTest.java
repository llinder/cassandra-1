...
public class DelimiterAnalyzerTest
{
    ...
    public void caseInsensitiveAnalizer() throws Exception
    {
        ...
        analyzer.init(
            new HashMap()
                {{
                    put(DelimiterTokenizingOptions.CASE_SENSITIVE, "false");
                }},
            UTF8Type.instance);
    ...
    {
        DelimiterAnalyzer analyzer = new DelimiterAnalyzer();
        analyzer.init(new HashMap(), UTF8Type.instance);
    ...
    {
        DelimiterAnalyzer analyzer = new DelimiterAnalyzer();   
        analyzer.init(new HashMap(), Int32Type.instance);
    ...
    {
        ByteBuffer bb = ByteBuffer.wrap(IOUtils.toByteArray(
                DelimiterAnalyzerTest.class.getClassLoader().getResourceAsStream("tokenization/french_skip_stop_words_before_stemming.txt")));
        ...
        while (tokenizer.hasNext())
        {
            ...
            words.add(org.apache.cassandra.serializers.UTF8Serializer.instance.deserialize(nextToken.duplicate()));
        }

        assertEquals(9, tokens.size());
        assertEquals("\"La", words.get(0));
        assertEquals("danse", words.get(1));
        assertEquals("sous", words.get(2));
        assertEquals("la", words.get(3));
    }
